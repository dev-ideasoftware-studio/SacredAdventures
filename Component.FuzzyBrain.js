...
class FuzzyBrain {
  ...
  update(delta) {
    ...
    if (now - this.lastReport > this.reportInterval) {
      this.lastReport = now;
      console.log(
        `[FuzzyBrain] FPS: ${this.smoothFPS.toFixed(1)} | Quality: ${this.qualityNames[this.qualityLevel]} | Shadows: ${this.shadows} | Res: ${this.pixelRatio.toFixed(1)}`,
      );
    }
  }
  ...
  setQuality(level) {
    ...
    this.cooldownFrames =
      this.qualityLevel < oldLevel ? 300 : this.cooldownDuration;

    console.log(
      `[FuzzyBrain] Quality: ${this.qualityNames[oldLevel]} → ${this.qualityNames[this.qualityLevel]}`,
    );
  ...
  // --- ANU DIAGNOSTIC INTERFACE ---
  // Called by Universe.Anu when mood shifts to 'restless'
  // Returns a plain-English summary of what FuzzyBrain thinks is wrong
  diagnose() {
    const fps = this.smoothFPS.toFixed(1);
    const q = this.qualityNames[this.qualityLevel];
    const reasons = [];

    if (this.smoothFPS < 15)
      reasons.push("FPS critically low — GPU likely overwhelmed");
    if (this.smoothFPS < 30)
      reasons.push(`FPS at ${fps} — below comfortable threshold`);
    if (!this.shadows)
      reasons.push("Shadows already disabled (quality degraded)");
    if (!this.pipEnabled) reasons.push("PIP disabled (SURVIVAL mode active)");
    if (this.aiThrottle >= 4)
      reasons.push(`AI throttled to every ${this.aiThrottle} frames`);
    if (this.maxVisibleTrees <= 20)
      reasons.push(`Trees reduced to ${this.maxVisibleTrees} visible`);
    if (this.pixelRatio < 1.0)
      reasons.push(
        `Resolution scaled to ${(this.pixelRatio * 100).toFixed(0)}%`,
      );

    const treeMeshCount = this.treeMeshes ? this.treeMeshes.length : "?";
    const npcCount = this.npcs ? this.npcs.length : "?";

    const summary =
      reasons.length > 0
        ? reasons.join("; ")
        : "No obvious single cause — may be accumulated draw calls";

    console.log(
      `%c[FuzzyBrain → Anu] Performance Report\n` +
        `  FPS: ${fps} | Quality: ${q} | AI Throttle: ${this.aiThrottle}x\n` +
        `  Trees in pool: ${treeMeshCount} | NPCs tracked: ${npcCount}\n` +
        `  PIP: ${this.pipEnabled ? "ON" : "OFF"} | Shadows: ${this.shadows ? "ON" : "OFF"} | Res: ${(this.pixelRatio * 100).toFixed(0)}%\n` +
        `  Diagnosis: ${summary}`,
      "color: #ffb74d; font-weight: bold;",
    );

    return { fps: this.smoothFPS, quality: q, reasons, summary };
  }
  ...
  applyPostProcessing(enable) {
    ...
    if (typeof window !== "undefined" && window.SacredState) {
      ...
      if (window.SacredState.vTiltShiftPass)
        window.SacredState.vTiltShiftPass.enabled = enable;
      if (window.SacredState.hTiltShiftPass)
        window.SacredState.hTiltShiftPass.enabled = enable;
    }
  }
  ...
  updateTreeLOD() {
    ...
    if (typeof window !== "undefined" && window.axePipCam)
      activePipCameras.push(window.axePipCam);
    if (typeof window !== "undefined" && window._nativeMapCam)
      activePipCameras.push(window._nativeMapCam);

    if (!this._pipFrustums) this._pipFrustums = [];
    while (this._pipFrustums.length < activePipCameras.length)
      this._pipFrustums.push(new THREE.Frustum());

    for (let i = 0; i < activePipCameras.length; i++) {
      this._pipProjMatrix.multiplyMatrices(
        activePipCameras[i].projectionMatrix,
        activePipCameras[i].matrixWorldInverse,
      );
    ...
    for (let i = 0; i < this.treeMeshes.length; i++) {
      ...
      if (t.isMesh || t.isGroup || t.isObject3D) {
        ...
        if (!this._dummySphere)
          this._dummySphere = new THREE.Sphere(new THREE.Vector3(), 8.0); // 8 unit radius covers most trees
    ...
    // --- CULL NPCs & WILDLIFE ---
    if (!this._npcDummySphere)
      this._npcDummySphere = new THREE.Sphere(new THREE.Vector3(), 5.0);
