...
export const ANU_PIPELINE_MEMORY = [
  ...
  },
  {
    ...
    learnedAt: "2026-05",
    title: "Fauna pillar — rabbit family burrow v4 (real hole with depth + bright rim + player-facing arc spawn)",
    summary:
      "js/v2/Fauna.js spawns a five-member rabbit family (dad 1.5× mom, mom, three babies with distinct coats) around a **minimal but legibly deep burrow**. **Location**: the empty tile **directly right of tipi 1** — WARREN_X = V2_TILE_WORLD ≈ 10.86 m, WARREN_Z = 0 — between tipi 1 (origin) and tipi 2 (V2_TILE_WORLD*2). Inside the V2_FLORA_TIPI_CLEAR_ZONE_RADIUS_M=12.5 m exclusion so no tree will plant on the ring. **Warren pieces** (`buildWarren()`): (a) disturbed-earth **RingGeometry** flush with the grass (outer 1.25 m, inner = throat top); (b) **fresh-soil bright rim** RingGeometry 0.542 → 0.61 m, lifted ~4 mm above the patch — defines the hole edge sharply against grass from 30+ m away (this is what makes 'real hole' read from spawn distance); (c) tapering open throat **0.55 → 0.20 m radius, 1.35 m deep** (was 0.38 → 0.22, 0.95 m in v3 — widened and deepened so depth is legible from oblique camera angles) with baked vertex-colour gradient rim soil (0x3a2316) → pure black on `MeshBasicMaterial({ vertexColors, side: BackSide, toneMapped: false })`; (d) inner **CircleGeometry** cap with radial vertex colours; (e) underground `BackSide` pocket for AI introspection. **Spawn arc**: the family scatters in a **130° arc on the south-facing grass** (player-facing side), radial 1.55–1.73 m — clearly *outside* the 1.25 m dirt patch on grass, not on the soil. Each rabbit faces the hole at boot. **Boot grace**: `state.stateTimer = 6 + i * 0.6` overrides the random idle timer set by `_createRabbitState`, holding all five in a calm IDLE pose for ~6–9 s (staggered) so the player sees a stationary family-by-the-hole tableau the instant the scene appears. After the grace the FSM resumes normally. **Animation gate**: action is created + scheduled (`action.play()`) at load but starts `paused = true`; `_updateRabbitMotion` flips `paused` based on locomoting state and bumps `timeScale` (CHASE 1.3×, FLEE 1.6×).",
    mitigations: [
      ...
      "**Bright soil rim (0x6b4530)** is what makes the hole read as a hole from spawn distance — without it, the grass→dark-soil contrast at 30 m oblique is too soft. If you ever want it more subtle, tune the colour, do not delete the geometry; spawn cam can otherwise miss the burrow entirely.",
      "**Spawn arc is south-facing** because the canonical first vantage is 3 tiles south of tipi 1 (see `world-collision-autowalk` for spawn). If a future spawn relocates to north of the warren, mirror the arc by flipping `dirZ = +Math.cos(angleFromSouth)` in `Fauna.load()`. Don't make the arc full 360° — the family-as-tableau read depends on the rabbits being silhouetted against grass, not against the dirt patch.",
      "**Boot grace is staggered (6–9 s)** so all five rabbits don't transition to HOP/GRAZE/CHASE in the same frame and create visual chaos at boot. The stagger also keeps the apparent motion sparse — only one rabbit is leaving idle at a time. Don't collapse to a shared timer.",
  ...
  },
  {
    ...
    mitigations: [
      ...
      "**NPC model yaw must NOT have the legacy `+ π` spin.** `V2_NPC_*_MODEL_YAW_RAD = π/2` (NOT `π/2 + π`). The seated aim helper `updateYellowButterflyPlayerAim` writes `facingGroup.rotation.y = atan2(dx, dz)` which aligns the facingGroup's local **+Z** with the vector to the player. For that to point the *model* at the player, the model's effective forward inside facingGroup must be +Z — and since the rig's natural forward is -X, `+π/2` is the exact correction. Adding another π flips her 180° and the user reports 'NPC faces wrong way'. Don't 'fix' it with a sign flip in the helper either; the helper's formula is correct.",
