...
window.BirdSystem = class BirdSystem {
  ...
  async init() {
    this.loader.load(
      ...
      (gltf) => {
        ...
        const hawk = THREE.SkeletonUtils
      ...
      (err) => {
        console.warn(
          "[BirdSystem] Bird.glb missing. Using procedural fallback.",
        );
  ...
  spawnGeeseFlock() {
    ...
    for (let i = 0; i < gooseCount; i++) {
      const goose = THREE.SkeletonUtils
        ? THREE.SkeletonUtils.clone(this.birdTemplate.scene)
        : this.birdTemplate.scene.clone();
  ...
  update(delta, params = {}) {
    ...
    this.solitaryBirds.forEach((bird) => {
      ...
      const activeRadius =
        bird.radius + Math.sin(time * 0.4 + bird.phase) * 6.0;
      const activeHeight =
        bird.height + Math.cos(time * 0.25 + bird.phase) * 3.5;

      const px = Math.cos(bird.angle) * activeRadius;
      const pz = Math.sin(bird.angle) * activeRadius;
      const futureAngle = bird.angle + 0.15;
      const futureRadius =
        bird.radius + Math.sin((time + 0.5) * 0.4 + bird.phase) * 6.0;
